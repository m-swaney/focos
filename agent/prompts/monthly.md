...
10. `## Cash flow and savings rate` income, spending, savings rate vs the profile.
...
16. `## Questions for {{OWNER}}` at most three questions whose answers would most improve this model
    (missing profile fields, unexplained flows, decisions still pending in `tax_agenda` or `goals.yml`).
    They answer periodically; ask only what matters.

Keep the monthly under 1,500 words. Append decisions to `state/decisions.jsonl` and return the
structured result.
